    ...
    def fractionalPacking(self, listofpairs, epsilon, budget, initx):
        ...
        for i in range(len(self.alledges)):
            if self.alledges[i] in usedbw:
                y.append(1 / float(self.alledges[i].capacity) * math.exp(
                    alpha * (float(usedbw[self.alledges[i]]) / float(self.alledges[i].capacity))))
        ...
        while maxratio >= lambda_0 / 2 and not self.determineP2(usedbw, self.alledges, y, minx, epsilon, maxratio,
                                                                budget):
            y = []
            #        for i in range(m):
